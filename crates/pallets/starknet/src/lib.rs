...
pub mod types;
/// The Starknet pallet's constans.
pub mod constants;
...
impl<T: Config> Pallet<T> {
    ...
    fn store_block(block_number: u64) {
        log!(info, "Welcome to store block");
    ...
    fn invoke_tick() -> Result<(), DispatchError> {
        ...
        //FIXME - Append to tick storage instead
        // Append the transaction to the pending transactions.
        Pending::<T>::try_append((transaction, receipt)).map_err(|_| Error::<T>::TooManyPendingTransactions)?;
    ...
    fn set_tick_tx() -> InvokeTransaction {
        ...
        InvokeTransaction {
            ...
            nonce, //FIXME - auto increment by 1
            signature: BoundedVec::try_from(signature).unwrap_or_default(),
            max_fee: Felt252Wrapper::from(u128::MAX), //FIXME - estimate fee
            is_query: false,
        }
    }
